  ...
  return (
        <header className="h-14 bg-white dark:bg-dark-800 border-b border-slate-200 dark:border-dark-700 flex items-center px-3 md:px-5 gap-3 md:gap-4 flex-shrink-0">
              ...
              <div className="hidden md:flex flex-col items-end">
                      <span className="text-xs font-bold text-dark-900 dark:text-white">{timeStr}</span>span>
                      <span className="text-xs text-slate-400 capitalize">{dateStr}</span>span>
              </div>div>
              ...
              <div className="flex items-center gap-2">
                      ...
                      <div className="hidden md:block">
                                <p className="text-xs font-bold text-dark-900 dark:text-white leading-none">{currentUser?.fullName || 'Admin'}</p>p>
                                <p className={`text-xs font-medium ${ROLE_COLORS[currentUser?.role || 'admin']}`}>{ROLE_LABELS[currentUser?.role || 'admin']}</p>p>
                      </div>div>
